...
class MZILinear(ONNBaseLayer):
    ...
    def reset_parameters(self) -> None:
        if self.mode == "weight":
            ...
            #     torch.empty(self.out_features, self.in_features, dtype=self.U.dtype, device=self.device)
            # )
            # W = torch.zeros(self.out_features, self.in_features, dtype=self.U.dtype, device=self.device)
            # print("linear初始化", W)
            U, S, V = torch.svd(W, some=False)
            V = V.transpose(-2, -1)

            std_dev = self.reset_phase_std
            print("linear_std初始化制造误差 :", std_dev)
            ...
            self.phase_U = self.phase_U + torch.randn(self.phase_U.size(), dtype=phi_mat.dtype, device=self.phase_U.device) * std_dev
            ...
            self.phase_V = self.phase_V + torch.randn(self.phase_V.size(), dtype=self.phase_V.dtype, device=self.phase_V.device) * std_dev
    ...
    def build_weight(self, update_list: set = {"phase_U", "phase_S", "phase_V"}) -> Tensor:
        if self.mode == "weight":
            ...
            #  这里的self.build_weight_from_phase会顺带更新self.U、self.S、self.V的值，模拟带误差的U、S、V矩阵
...
class MZIBlockLinear(ONNBaseLayer):
    ...
    def reset_parameters(self) -> None:
        if self.mode == "weight":
            ...
            U, S, V = torch.svd(W, some=False)
            V = V.transpose(-2, -1)
            # U, S, V = torch.svd(W, full_matrices=True, driver="gesvd")  # must use QR decomposition
            self.U.data.copy_(U)
            self.V.data.copy_(V)
            self.S.data.copy_(S)
        elif self.mode == "phase":
