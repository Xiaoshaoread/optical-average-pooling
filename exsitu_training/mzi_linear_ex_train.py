...
class MZILinear(ONNBaseLayer):
    ...
    def build_weight(self, update_list: set = {"phase_U", "phase_S", "phase_V"}) -> Tensor:
        if self.mode == "weight":
            ...
            S = self.S
            # print('self.U',self.U)
            self.weight = self.build_weight_from_usv(U, S, V)
            # print("build_weight_from_usv", self.weight)
            self.delta_list_U, self.phase_U, self.delta_list_V, self.phase_V, self.phase_S, self.S_scale = self.build_phase_from_weight(
                self.weight)
            ...
            if self.phase_noise_std > 1e-5:
                ### phase_S is assumed to be protected
                self.phase_U = self.phase_U + gen_gaussian_noise(
                    self.phase_U,  # 待加入噪声的相位
                    0,  # 噪声均值
                    self.phase_noise_std,  # 噪声标准差
                    trunc_range=(-2 * self.phase_noise_std, 2 * self.phase_noise_std),
                )
                self.phase_V = self.phase_V + gen_gaussian_noise(
                    self.phase_V,
                    0,
                    self.phase_noise_std,
                    trunc_range=(-2 * self.phase_noise_std, 2 * self.phase_noise_std),
            ...
            weight = self.build_weight_from_phase(
                self.delta_list_U, self.phase_U, self.delta_list_V, self.phase_V, self.phase_S, update_list=update_list
            )
            # print("加噪后", self.U)
            # print("加噪后weight", self.weight)
            # print("S",S)
        return weight
