...
class MZIConv2d(ONNBaseLayer):
    ...
    def build_parameters(self, mode: str = "weight") -> None:
        ...
        # 《小邵》：使用 Parameter 来包装模型参数，以便于在模型优化过程中对其进行自动求导、梯度更新等操作。
        if mode == "weight":
    ...
    def build_weight(self, update_list: set = {"phase_U", "phase_S", "phase_V"}) -> Tensor:
        if self.mode == "weight":
            ...
            self.delta_list_U, self.phase_U, self.delta_list_V, self.phase_V, self.phase_S, self.S_scale = self.build_phase_from_weight(self.weight)
