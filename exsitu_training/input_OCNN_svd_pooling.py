...
for std_all in np.arange(0.03, 0.031, 0.001):
    ...
    for i in range(num):
                        ...
                        for i in range(7):  # 每行迭代
                            for j in range(7):  # 每列迭代
                                ...
                                Y[m, n, i, j] = torch.tensor(
                                    np.dot(noisy_U_grid, x[m, n, 4 * i:4 * i + 4, 4 * j:4 * j + 4].reshape(16, 1)))
                Y = Y / (4)
                # print("Y:" + str(Y))
                x = Y.flatten(1)
                x = torch.relu(self.linear(x))
        ...
        #  对池化层MZI相位引入噪声
        noisy_U_grid = (add_gaussian_noise_to_phase(U_grid, std_pooling).T)[0]
