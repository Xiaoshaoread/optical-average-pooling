...
class phase_quantize_fn_cpu(object):
    ...
    def __call__(self, x):
        if self.p_bit == 32:
            ...
            E = np.mean(np.abs(x))  # 对x取绝对值后算平均值
...
def voltage_to_phase_cpu(voltages, gamma):
    ...
    return phases
    # 邵：将电压值先平方，然后再乘以 gamma 系数，最后取模 2π 得到一个相位值。最后将相位值限制在 [-π, π] 的范围内。
...
def phase_to_voltage_cpu(phases, gamma):
    ...
    return voltages
    # 邵：如果输入的是一个numpy数组，函数首先将相位变换为[-pi, pi]的范围，然后根据给定的gamma值计算出一个电压矩阵。
    # 如果输入的是一个pandas.Series对象，则直接用相位进行计算
...
def vector_to_checkerboard(vec, complex=False):
    ...
    if isinstance(vec, np.ndarray):
        ...
        M = vec.shape[-1]  # M为vec的列数
        N = (1 + int(np.sqrt(1 + 8 * M))) // 2  # sqrt:平方根 //：向下取整
        upper_oddN = N - (N % 2 == 1)  # N为偶数时upper_oddN=N，为奇数时upper_oddN=N-1
        upper_evenN = N - (N % 2 == 0)  # N为偶数时upper_evenN=N-1，为奇数时upper_evenN=N
        vector_even_col = vec[..., : (N // 2) * ((N + 1) // 2)]
        vector_odd_col = vec[..., (N // 2) * ((N + 1) // 2):]
        mat = np.zeros([*vec.shape[:-1], N, N], dtype=vec.dtype)  # mat是维数为（vec.shape[:-1],N,N)的全零矩阵
...
def project_matrix_to_unitary(W):  # 将矩阵转化为酉矩阵
    if isinstance(W, np.ndarray):
        U, _, V = np.linalg.svd(W, full_matrices=True)  # svd分解
        U_refine = np.matmul(U, V)  # UV相乘
...
class PhaseQuantizer(torch.nn.Module):
    ...
    def __init__(
        ...
        Args:
            ...
            gamma_noise_std (float, optional): std dev of Gaussian phase noise on the gamma coefficient. Defaults to 0. 高斯相位噪声。默认为0。
    ...
    def set_gamma_noise(self, noise_std: float, size: _size, random_state: Optional[int] = None):
        ...
        # 《小邵》：如果指定了随机种子，就将 PyTorch 设定为确定性模式
        if random_state is not None:
            set_torch_deterministic(random_state)

        # 《小邵》：生成高斯噪声，并将其添加到原始的 gamma 参数上，得到带有噪声的参数 $\tilde{\gamma}$
        self.noisy_gamma = (
    ...
    def forward(self, x):
        x = x % (2 * np.pi)
        # 《小邵》：对输入 x 的每一个元素进行取模运算，使其限制在0到2pi的范围内
        ...
        # 《小邵》：当 self.noisy_gamma 不为 None 时，x 会乘以 self.noisy_gamma 除以 self.gamma，
        #  所以 x 可以看作是加入了 Gamma 噪声处理的后的数据

        if self.crosstalk_factor > 1e-5:
            x = self.crosstal_simulator.simple_simulate(x, mixedtraining_mask=None, mode=self.mode)
        # 《小邵》：当 self.crosstalk_factor 大于 1e-5 时，x 还会经过热耦合模拟器 self.crosstal_simulator 的处理
        #  因此 x 也会综合考虑热耦合的影响
